...
func _update_state(delta: float) -> void:
	...
	var target_position := _get_target_position() if has_target else global_position
	var distance_to_target := _horizontal_distance(global_position, target_position)
	match state:
		State.IDLE:
			_slow_horizontal(delta, acceleration)
			if has_target and distance_to_target <= aggro_range:
				_set_engaged(true)
				_change_state(State.CHASE)
		State.CHASE:
			if not has_target or distance_to_target > disengage_range:
				_set_engaged(false)
		...
		State.RECOVERY:
			...
			if state_time <= 0.0:
				_change_state(State.CHASE if has_target else State.IDLE)
		State.STAGGER:
...
func _chase_target(target_position: Vector3, delta: float) -> void:
	...
	if navigation_refresh <= 0.0:
		navigation_refresh = 0.2
		navigation_agent.target_position = target_position
	var direction := _safe_navigation_direction(target_position)
