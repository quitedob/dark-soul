...
func _recover(player: Node) -> void:
	...
	_claimed = true
	monitoring = false
	var handled := _call_first(world, [&"recover_lost_echo", &"recover_embers", &"add_embers"], [amount, player])
	if not handled:
		_call_first(player, [&"recover_embers", &"add_embers", &"add_souls", &"add_currency"], [amount])
