# game/scripts/boss/boss_attack_projectile.gd
extends Area3D
## L-19：Boss 导向投射物（homing_projectile 用）。可平飞可追踪，命中走项目受击契约。
## 参照 enemy_projectile.gd 的扫掠防穿透实现，附加每帧向目标转向。属于 scripts/boss 下的
## 独立 helper（不影响 enemy_projectile.gd / scenes 资源）。
...
func setup(
	...
	speed = float(metadata.get("proj_speed", 9.0))
	lifetime = float(metadata.get("proj_lifetime", 3.0))
	var ht: Variant = metadata.get("homing_target")
	homing_target = ht if ht is Node3D else null
	homing_strength = float(metadata.get("homing_strength", 3.0))
	_collision_radius = float(metadata.get("collision_radius", 0.2))
	hit_payload = {
		"damage": damage,
		"stagger": stagger,
		"poise": stagger,
		...
		"direction": direction,
		"source": source,
		"action_id": String(metadata.get("action_id", "boss_homing_projectile")),
		"tags": metadata.get("tags", ["projectile", "enemy", "boss"]).duplicate(),
...
func _steer(delta: float) -> void:
	if homing_target == null or not is_instance_valid(homing_target):
		return
	var desired := homing_target.global_position - global_position
	desired.y = 0.0
	...
	var merged := current.lerp(desired, blend)
	if merged.length_squared() < 0.001:
		return
	direction = merged.normalized()


## 球形扫掠：世界阻挡或命中玩家（与 enemy_projectile 一致）
