...
extends CanvasLayer
## Death screen overlay — fades in with "EMBER FADED" message.
...
func _build_overlay() -> ColorRect:
	...
	overlay.color = Color(0.027, 0.004, 0.004, 0.88)
	...
	var title_label := _theme.make_label("EMBER FADED", 54, Color("eb6b4a"))
	...
	var subtitle_label := _theme.make_label("Your embers scatter into the hollow mist.", 14, HudTheme.COLOR_MUTED)
