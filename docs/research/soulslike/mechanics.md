...
> **RESEARCH ARCHIVE.** §9 PENDING 列表已过时；实现状态见 [devlog](../../devlog/index.md) / [调研汇总](../index.md)。

**Date:** 2026-07-30  
**Status:** `ARCHIVED`

Research conducted 2026-07-30. This report provides frame-level detail on Dark Souls combat mechanics — i-frame durations, parry window frame counts, poise health mathematics with worked examples, attribute scaling curves, and corresponding Godot implementation patterns. It complements the broader design audit in [`design.md`](design.md) with the precise numerical and architectural depth needed to implement these systems in GDScript.
...
**Ashen Hollow relevance:** Current implementation has no poise/hyper-armor system. This is documented as a pending recommendation in [`weapons.md`](weapons.md) (Section 11, recommendation #5). Twin Colossi style should be unstaggerable during active frames — this is the primary reward for surviving the long wind-up.
...
**Ashen Hollow relevance:** All five combat styles currently use uniform timing and costs. Per-style `Resource`-based differentiation is the highest-priority pending recommendation (see [`weapons.md`](weapons.md), Section 11).
...
1. **Per-style attack timing differentiation** (PENDING) — Currently all five styles use uniform wind-up/active/recovery durations. Twin Colossi must feel dramatically slower than Crescent Pair. See [`weapons.md`](weapons.md) Section 11, Tuning Reference table.
