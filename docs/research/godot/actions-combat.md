...
**Status:** `ARCHIVED`  
**项目：** 烬渊 / Ashen Hollow  
**目标版本：** Godot 4.7.1-stable

## 核心结论

### 高置信度

1. 项目已经具备数据驱动招式层：`AttackData`、`MovesetData`、`WeaponData`、`WeaponArtData`、`GuardProfile` 和兼容工厂均已存在。
2. ~~战斗仅靠代码计时 FSM、无 AnimationTree~~ → **已有** `PlayerAnimationBridge`（占位 AnimationTree + method-track + RM）；真蒙皮资产仍缺。
3. ~~仅 150ms 单槽缓冲~~ → **已有** 多槽 Action Queue（B-09）；HUD 仍可读兼容单槽字段。
4. `CombatArea` 统一命中载荷与挥击去重；ShapeCast 连续采样已接。
5. Godot 不强制节点式 HFSM；项目选择「逻辑权威 + 动画事件适配」。
6. 迁移原则：招式数据与游戏逻辑保持权威；动画只发语义事件。
...
> 下表为 2026-07-30 调研快照。**现网**以 [devlog](../../devlog/index.md) / `tasks-master.md` / `architecture.md` 为准。已删除的 `docs/tasks/*` 规格不再维护。

| 文档 | 现网备注 |
|---|---|
| `systems/attack-moveset-data-schema.md` | Resource 所有权仍有效 |
| `systems/combat-execution-guard-weapon-arts.md` | 目标权威；多数已落地 |
| `tasks/combat-expansion-roadmap.md` | 已精简为里程碑对照 |
| `architecture.md` / `controls.md` | 2026-07-31 已同步 |
| `tasks-master.md` | 仅列开放项 + 维度摘要 |
| ~~`tasks/d-01-*` / `e-04-*`~~ | 已归档删除 |
